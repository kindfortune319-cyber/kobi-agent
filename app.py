...
if prompt := st.chat_input("Какую задачу нужно решить? (например: 'Сделай структуру презентации' или 'Напиши КП')"):
    ...
    with st.chat_message("assistant"):
        with st.status("Kobi анализирует задачу и формирует документ...", expanded=True) as status:
            ...
            system_prompt = (
                ...
                "2. [PDF] — ЕСЛИ ПОЛЬЗОВАТЕЛЬ ПРОСИТ ДОКУМЕНТ, ПРЕЗЕНТАЦИЮ, СТРУКТУРУ, СОЧИНЕНИЕ, КП ИЛИ ОТЧЕТ. Весь текст после тега [PDF] пойдет в Word-документ.\n"
            ...
            # Страховка на случай, если модель забыла тег, но пользователь просил документ или презентацию
            if "[PDF]" not in full_reply and any(word in prompt.lower() for word in ["pdf", "файл", "презентац", "структур", "сочинение", "предложение", "отчет", "документ", "смет"]):
                full_reply = "[PDF]\n" + full_reply
